...
class TweetVacTestCase(unittest.TestCase):
    ...
    def test_suck_should_stop_when_no_data(self):
        ...
        data = self.tweetvac.suck(endpoint)

        self.assertEqual(2, len(data))

    @responses.activate
    def test_suck_should_respect_max_requests(self):
        endpoint = 'statuses/user_timeline'
        url = self.createUrl(endpoint, {})
        body = b'[{"id": 200}, {"id": 100}]'
        responses.add(responses.GET, url, body=body, content_type='application/json')

        data = self.tweetvac.suck(endpoint, max_requests=5)

        self.assertEqual(10, len(data))

    @responses.activate
    def test_suck_with_filters(self):
        endpoint = 'statuses/user_timeline'
        url = self.createUrl(endpoint, {})
        ...
        data = self.tweetvac.suck(endpoint, filters=[f1, f2], max_requests=1)
    ...
    @responses.activate
    def test_suck_with_cutoff(self):
        endpoint = 'statuses/user_timeline'
        url = self.createUrl(endpoint, {})
        body = b'[{"id": 200}, {"id": 100}]'
        responses.add(responses.GET, url, body=body, content_type='application/json')
        ...
        data = self.tweetvac.suck(endpoint, cutoff=f)
